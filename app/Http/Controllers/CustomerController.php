...
class CustomerController extends Controller
{
	...
	public function postSignup(Request $request){
        //tao ham gui email
        function sendMail($stringFullName,$stringEmail){
            $data = array("name" => $stringFullName);

            Mail::send('frontend.send_mail',$data,function($message) use ($stringFullName,$stringEmail){
                $message->to($stringEmail)->subject("Verify Mail");
                $message->from($stringEmail,"Luxury Watches");
            });
        }
		$request->validate([
		...
		$customer->save();
		sendMail($request->stringFullName,$request->stringEmail);
		return back()->with(['typeMsg'=>'info','msg'=>'Chúng tôi đã gửi cho bạn một email xác thực']);
	}
    //backend
